...
from .voxceleb import Vox
from .bupt import BUPTDataset
from .celebvhq import CelebV_hq
...
def build_train(config, is_train=True):
    ...
    if 'mead' in config.training_data:
        data_list.append(MEAD(K=config.K, image_size=config.image_size, scale=[config.scale_min, config.scale_max], trans_scale=config.trans_scale, isSingle=config.isSingle))
    if 'celebvhq' in config.training_data:
        data_list.append(CelebV_hq(K=config.K, image_size=config.image_size, scale=[config.scale_min, config.scale_max], trans_scale=config.trans_scale, isSingle=config.isSingle))
    if 'voxceleb' in config.training_data:
        data_list.append(Vox(K=config.K, image_size=config.image_size, scale=[config.scale_min, config.scale_max], trans_scale=config.trans_scale, isSingle=config.isSingle))
    if 'bupt' in config.training_data:
        data_list.append(BUPTDataset(K=config.K, image_size=config.image_size, scale=[config.scale_min, config.scale_max], trans_scale=config.trans_scale, isSingle=config.isSingle))
    if 'celebvhq' in config.training_data:
        data_list.append(CelebV_hq(K=config.K, image_size=config.image_size, scale=[config.scale_min, config.scale_max], trans_scale=config.trans_scale, isSingle=config.isSingle))
...
def build_val(config, is_train=True):
    ...
    if 'mead' in config.training_data:
        data_list.append(MEAD(K=config.K, image_size=config.image_size, scale=[config.scale_min, config.scale_max], trans_scale=config.trans_scale, isSingle=config.isSingle, sep_num=(-10,-1)))
    if 'celebvhq' in config.training_data:
        data_list.append(CelebV_hq(K=config.K, image_size=config.image_size, scale=[config.scale_min, config.scale_max], trans_scale=config.trans_scale, isSingle=config.isSingle, sep_num=(-100,-1)))
    if 'voxceleb' in config.eval_data:
        data_list.append(Vox(K=1, image_size=config.image_size, scale=[config.scale_min, config.scale_max], trans_scale=config.trans_scale, isSingle=True, sep_num=(-1000,-1)))
    if 'bupt' in config.eval_data:
        data_list.append(BUPTDataset(K=1, image_size=config.image_size, scale=[config.scale_min, config.scale_max], trans_scale=config.trans_scale, isSingle=True, sep_num=(-10000,-1)))
