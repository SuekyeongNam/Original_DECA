...
class MEAD(Dataset):
    def __init__(self, K, image_size, scale, trans_scale = 0, isTemporal=False, isEval=False, isSingle=False, sep_num=(0,-10)):
        '''
        K must be less than 6
        '''
        self.K = K
        self.image_size = image_size
        self.imagefolder = '/data/vision_team/MEAD_sampled/images'
        self.kptfolder = '/data/vision_team/MEAD_sampled/kpt'
        self.segfolder ='/data/vision_team/MEAD_sampled/segmentation'
        self.pklfolder ='/data/vision_team/MEAD_sampled/flame'

        self.pkllist = glob(self.pklfolder + '/*.pkl')[sep_num[0]:sep_num[1]]
        self.datalist = []
        for fname in tqdm(self.pkllist) :
            kpt_name = fname.replace('flame','kpt')[:-3]+'npy'
            image_name = fname.replace('flame','images')[:-3]+'jpg'
            seg_name = fname.replace('flame','segmentation')[:-3]+'png'
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.K):
            ...
            pkl_path = os.path.join(image_path.replace('images','flame')[:-3] + 'pkl')
